...
<div class="editable_field_block panel panel-default">
    <div class="panel-body">
        <ul class="list-group gp_files_list">
            {if isset($object.gp_files) && !empty($object.gp_files)}
                {foreach from=$object.gp_files item=file}
                    {if !isset($file.gp.link) || empty($file.gp.link)}{continue}{/if}
                        <li class="list-group-item">
                            <a class="" href="/{$file.gp.link}" >{$file.gp.full_name}</a>
                            <span style="margin-left: 10px;" title="Удалить файл:::admin/gp_files/delete_file.json?id={$file.id}::file-preview-{$field_name}-{$file.id}::gp::{$file.gp.full_name}:::" class="c_r delete_file_handler btn btn-xs btn-danger">удалить</span>
                        </li>
                {/foreach}
            {/if}
        </ul>
    ...
    <div class="image_btn_container">
        <div style="position: relative; height: 24px;  width: 65px;">
            <div class="select_file_container">
                ...
                <input id="fileupload" class="select_file_input input_file editable_field" type="file" name="gp[]" multiple />
            </div>
        </div>
    </div>
    <br>
    <br>
    <div style="width: 400px;" id="progress" class="progress">
...
{literal}
    <script type="text/javascript">
        ...
        $(function () {
            ...
            var url = '/admin/'+$('.module_name').attr('value')+'/upload_file/?id='+$('.field_key').attr('value')+'&model='+$('.field_model').attr('value');
            $('#fileupload').fileupload({
                url: url,
                dataType: 'json',
                done: function (e, data) {
                    var new_file = '<li class="list-group-item"><a class="" href="/'+data.result.link+'" >'+data.result.title+'</a></li>'
                    $('.gp_files_list').append(new_file);
                },
